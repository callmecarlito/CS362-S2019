...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return adventurer_call(handPos, currentPlayer, state, &drawntreasure, &cardDrawn, &z);
      			
    case council_room:
      return council_room_call(handPos, currentPlayer, state);
    ...
    case smithy:
       return smithy_call(handPos, currentPlayer, state);      
		
    case village:
        return village_call(handPos, currentPlayer, state);
    ...
    case cutpurse:
      return cutpurse_call(handPos, currentPlayer, state);
...
{
  //+3 Cards
  for (int i = 0; i < 4; i++) //introduced error here; 4 cards will be drawn instead of 3
    {
    drawCard(currentPlayer, state);
    }		
  //discard card from hand
...
int council_room_call(int handPos, int currentPlayer, struct gameState *state)
{
  ...
  for (int i = 0; i < state->numPlayers; i++)
    {
	if ( i == currentPlayer ) //introduced bug here, switched != to == ......
	  {
	  drawCard(i, state);
	  }
	}
...
int cutpurse_call(int handPos,int currentPlayer, struct gameState *state)
{
  ...
  //discard played card from hand
  //discardCard(handPos, currentPlayer, state, 0); introduced bug by commenting out discardCard function call	
  return 0;
}
...
int village_call(int handPos, int currentPlayer, struct gameState *state)
{
  ...
  //+2 Actions
  state->numActions = state->numActions + 100; //introduced bug where actions are +100 instead of +2 
